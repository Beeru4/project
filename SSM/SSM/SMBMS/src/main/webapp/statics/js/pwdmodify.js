...
$(function(){
	...
	oldpassword.on("blur",function(){
		if($(this).val == null || "" == $(this).val().trim()){
			validateTip(oldpassword.next(),{"color":"red"},imgNo + " 请输入密码",false);
			return;
		}
		...
		$.ajax({
			...
			url:path+"/sys/user/checkedOldPwd.json",
			data:{oldpassword:oldpassword.val()},
			dataType:"json",
			success:function(data){
				if(data.key.result == "true"){//旧密码正确
					validateTip(oldpassword.next(),{"color":"green"},imgYes,true);
				}else if(data.key.result == "false"){//旧密码输入不正确
					validateTip(oldpassword.next(),{"color":"red"},imgNo + " 原密码输入不正确",false);
				}else if(data.key.result == "sessionerror"){//当前用户session过期，请重新登录
					validateTip(oldpassword.next(),{"color":"red"},imgNo + " 当前用户session过期，请重新登录",false);
				}else if(data.key.result == "error"){//旧密码输入为空
