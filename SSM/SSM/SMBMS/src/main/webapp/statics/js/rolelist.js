...
function deleteProvider(obj){
	$.ajax({
		...
		data:{roleId:obj.attr("proid")},
		dataType:"json",
		success:function(data){
			if(data.result.code == 1000){//删除成功：移除删除行
				cancleBtn();
				//obj.parents("tr").remove();
				location.reload();
			}else if(data.result.code == 1001){//删除失败
				//alert("对不起，删除供应商【"+obj.attr("proname")+"】失败");
				changeDLGContent("对不起，删除角色【"+obj.attr("proname")+"】失败,该角色已被其他用户应用!");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
        ...
        var obj = $(this);
        // "/jsp/provider.do?method=view&proid="+ obj.attr("proid");
		window.location.href=path+"/sys/role/roleView/"+ obj.attr("proid");
	});
	...
	$(".deleteProvider").on("click",function(){
		providerObj = $(this);
		changeDLGContent("你确定要删除角色【"+providerObj.attr("proname")+"】吗？");
		openYesOrNoDLG();
	});
