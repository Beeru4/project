	...
	$.ajax({
		...
		success:function(data){
			if(data.key.delResult == "true"){//删除成功：移除删除行
				...
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.key.delResult+"】条订单，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		window.location.href=path+"/sys/provider/viewProvider/"+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		/*window.location.href=path+"/jsp/provider.do?method=modify&proid="+ obj.attr("proid");*/
		window.location.href=path+"/sys/provider/viewBefore.html?proid="+ obj.attr("proid");
	});
