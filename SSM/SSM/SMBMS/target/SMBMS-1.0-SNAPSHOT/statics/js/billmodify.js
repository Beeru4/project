...
$(function(){
	...
	$.ajax({
		...
		url:path+"/sys/bill/billAddBefore.json",//请求的url
		...
		success:function(data){//data：返回数据（json对象）
			if(data != null){
				...
				for(var i = 0; i < data.providers.length; i++){
					...
					if(pid != null && pid != undefined && data.providers[i].id == pid ){
						options += "<option selected=\"selected\" value=\""+data.providers[i].id+"\" >"+data.providers[i].proName+"</option>";
					}else{
						options += "<option value=\""+data.providers[i].id+"\" >"+data.providers[i].proName+"</option>";
					}
