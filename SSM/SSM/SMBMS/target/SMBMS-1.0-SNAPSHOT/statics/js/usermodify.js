...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
        url:path+"/sys/user/getRole.json",
		...
		success:function(data){//data：返回数据（json对象）
			if(data != null){
				...
				for(var i = 0; i < data.roles.length; i++){
					...
					if(rid != null && rid != undefined && data.roles[i].id == rid ){
						options += "<option selected=\"selected\" value=\""+data.roles[i].id+"\" >"+data.roles[i].roleName+"</option>";
					}else{
						options += "<option value=\""+data.roles[i].id+"\" >"+data.roles[i].roleName+"</option>";
					}
